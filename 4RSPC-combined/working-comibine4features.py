...
def run_patch_update(ip, username, password, apply_patches=False):
    ...
    try:
        ...
        for cmd in commands:
            ...
            # If command output indicates available updates
            if os_type in ["ubuntu", "debian"] and "upgradable" in output:
                update_needed = True
            elif os_type in ["centos", "rhel"] and "Updated Packages" in output:
                update_needed = True
            elif os_type in ["suse", "sles"] and "No updates found" not in output:
                update_needed = True
