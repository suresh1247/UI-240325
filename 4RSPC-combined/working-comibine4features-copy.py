...
def run_patch_update(ip, username, password, apply_patches=False):
    ...
    try:
        ...
        if os_type == "ubuntu" or os_type == "debian":
            # Suppress apt warnings by setting environment variable DEBIAN_FRONTEND=noninteractive
            commands = [
            ...
            commands = [
                "sudo yum clean all",
                "sudo yum makecache",
                "sudo yum check-update || true"  # Ignore non-zero exit code when updates are available
            ]
            if apply_patches:
                commands.append("sudo yum update -y")
        elif os_type == "suse" or os_type == "sles":
            commands = [
                "sudo zypper refresh",
        ...
        for cmd in commands:
            ...
            # Ignore apt warnings (non-critical)
            if os_type in ["ubuntu", "debian"] and "WARNING: apt does not have a stable CLI interface" in error:
                error = ""  # Suppress the warning

            # Ignore yum check-update non-zero exit code (updates available)
            if os_type in ["centos", "rhel"] and cmd == "sudo yum check-update || true":
                if "Error:" not in error:  # Only treat as an error if there is a real error
                    error = ""
            ...
            if os_type in ["ubuntu", "debian"]:
                # Check if there are any upgradable packages in the output
                if "upgradable" in output and "Listing..." in output:
                    # Extract the list of upgradable packages
                    upgradable_packages = [line for line in output.splitlines() if "/" in line]
                    if upgradable_packages:  # If there are upgradable packages
                        update_needed = True
            elif os_type in ["centos", "rhel"]:
                if "Updated Packages" in output:  # Check for updated packages
                    update_needed = True
            elif os_type in ["suse", "sles"]:
                if "No updates found" not in output:  # Check if updates are found
                    update_needed = True
        ...
        if update_needed:
            if apply_patches:
                ...
                return (ip, "✅ Patches Available", "blue", artifact_filename, None)
        else:
            return (ip, "✅ Server Up to Date", "green", artifact_filename, None)
