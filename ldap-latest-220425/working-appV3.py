...
def run_command(ip, action, username, password):
    ...
    try:
        ...
        for cmd in commands:
            stdin, stdout, stderr, exit_code = log_command(ssh, cmd, username, ip, action)
...
def check_reboot_required(ssh, username, ip):
    stdin, stdout, stderr, exit_code = log_command(ssh, "[ -f /var/run/reboot-required ] && echo Yes || echo No", username, ip, 'health_check')
    try:
...
def get_kernel_version(ssh, username, ip):
    stdin, stdout, stderr, exit_code = log_command(ssh, "uname -r", username, ip, 'health_check')
    try:
        return stdout.read().decode().strip()
    except:
        return 'Error'
